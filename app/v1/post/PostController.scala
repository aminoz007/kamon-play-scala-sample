...
import v1.time.RemoteTimeClient
...
/**
  ...
  */
class PostController @Inject()(cc: PostControllerComponents, timeClient: RemoteTimeClient)(
  implicit ec: ExecutionContext)
  extends PostBaseController(cc) {

  private val logger = Logger(getClass)
  implicit val indexResponseWrites = Json.writes[IndexResponse]
  ...
  def index: Action[AnyContent] = PostAction.async { implicit request =>
    logger.trace("index: ")
    val res = for {
      p <-  postResourceHandler.find
      t <- timeClient.fetch
    } yield (p, t )

    res.map(tuple => {
      Ok(Json.toJson(IndexResponse(tuple._1, tuple._2)))
    })
  }
